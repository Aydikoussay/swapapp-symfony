...
- Results from Security Pipeline #8 (12 Sep 2026, pre-remediation):
  ...
  - ZAP baseline: **31 URLs, all checks PASS** — yet job 9 went RED twice on pure infra bugs: first the action's default `artifact_name: zap_scan` was rejected, then the renamed `dast-zap-scan` too → the action's built-in uploader itself is broken (retired artifact API). Fixed by dropping the action: direct `docker run ... zap-baseline.py` with identical flags, reports stored by our own `upload-artifact@v4` step (see workflow comment).
- **F5 — DAST target returned HTTP 500 (app couldn't boot in CI):** ZAP's spider logged `status code 500 expected 200` on `/`; the WARNs (debug error messages, version leak, missing CSP) were mostly Symfony error-page artifacts. Cause: every page queries Doctrine, but no DB server exists in CI (`.env` points at `127.0.0.1:3306` MySQL). Fix (Dockerfile only, app untouched): boot the CI image on a sqlite file DB (`DATABASE_URL="sqlite:////var/www/html/var/ci.db"`, schema created from entities since the repo has no migrations). Lesson: DAST is only as good as its target — a 500 homepage means you're scanning error pages.
